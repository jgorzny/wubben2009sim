...
class BayesActor(object):
    ...
    #learn_prop_init = [0.05, 0.15, 0.2, 0.6]  #the self is bit more scroogy, but don't know the other at all
        ...
        learn_agent = PDAgentB(N=self.num_samples,alpha_value=1.0,
...
#Game methods
    def takeTurn(self, turnNumber):
        ...
        if(coopDist < defectDist):
            #cooperate
            percent = self.cosine_similarity(self.coopVect, learn_aab)
            toGive = round(percent * 10)
        else:
            #defect
            percent = self.cosine_similarity(self.defectVect, learn_aab)
            toGive = round((1-percent) * 10)
    ...
    def updateLastAction(self, numReceived):
        ...
        print "BayesActor: " + str(self.last_aab) + " and " + str(self.last_paab) + " and " + str(self.learn_avgs)
        ...
        print "about to prune",learn_observ,learn_xobserv
    ...
    def receiveMessage(self, intensity, emotion):
        print "Extra message received."
        result = [0,0,0] #temp; never used
        ...
        '''
        #self.learn_avgs=self.agent.propagate_forward(self.last_aab,[],result,self.last_paab)
        
        
        pre_agent_avgs = self.agent.getAverageState()
        (aid,cid)=self.agent.get_avg_ids(pre_agent_avgs.f)
        print "BayesActor: Before Message: Agent thinks it was most likely a: ",aid
        print "BayesActor: Before Message: Agent thought you were most likely a: ",cid



        
        #we're taking an extra round of turns here where the agent 'waits' and the client sends the message
        
        learn_xobservb = [State.turnnames.index("client"),0]
        learn_xobservb[1] = 0        
        self.learn_avgs=self.agent.propagate_forward([0,0,0],[],learn_xobservb,0)
        
        learn_xobserv = [State.turnnames.index("agent"),0] #was agent here
        learn_xobserv[1] = 0
        self.learn_avgs=self.agent.propagate_forward([],result,learn_xobserv,0)
        
        
        post_agent_avgs = self.agent.getAverageState()
        (aid,cid)=self.agent.get_avg_ids(post_agent_avgs.f)
        print "BayesActor: After Message: Agent thinks it is most likely a: ",aid
        print "BayesActor: After Message: Agent thought you are most likely a: ",cid        
             
class PDAgentB(Agent):
    def __init__(self,*args,**kwargs):

        super(PDAgentB, self).__init__(self,*args,**kwargs)
    ...
    def observationDist(self,obs1,obs2):
        ...
        print obs2[0]
        print "ccc"
        print str(self.pomcp_agent.ucTree.root.actionSet[0])
        '''
